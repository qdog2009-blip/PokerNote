...
全部玩家结算后，场次明细会显示系统计算的抽水总额，并允许有录入权限的用户手动保存“最终抽水”。最终抽水必须是大于等于 `0` 的整数；单场统计、分组汇总、玩家净结算总额和水池都以该金额为准，玩家逐项明细仍保留按抽水比例计算的结果供核对。如果之后修改玩家、买入、结算或抽水比例，旧的手动最终抽水会自动清除并重新计算。

全部玩家结算完成后，系统会自动把账目误差并入水池：

```text
结算误差 = 总买入 - 抽水前结算总额
最终抽水 = 手动保存金额（未手动保存时为盈利玩家抽水合计）
最终水池 = 最终抽水 + 结算误差
```
