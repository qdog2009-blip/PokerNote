...
3. 进入独立的“创建新场次”页面，选择分组并设置标题、抽水比例和统一带入金额
4. 从全部历史玩家中多选本场玩家，保存后一次创建场次、玩家和相同金额的首次买入
5. 买入操作（💰 买入 → 选玩家 → 输入金额）
...
- 新场次标题默认使用当天 `MMDD`，抽水比例默认带入当前账号上一次保存的值
- 创建页会列出当前账号可访问场次中的全部历史玩家，支持多选、全选和清空
- 点击“保存并创建”前不会写入场次；保存时场次、所选玩家及统一首次买入会在同一数据库事务中创建
- 可以创建多个自定义分组，并随时修改场次所属分组
- 场次列表按分组分段显示，每个分组可独立查看总买入、结算、抽水、净结算和账目误差
- 同一分组内同名玩家会跨场次合并，按抽水前总战绩倒序显示“参与场数、水上场数、抽水前总战绩”；总战绩相同时按参与场数倒序，未结算玩家排在最后
...
全部玩家结算后，场次明细会把“系统抽水 + 结算误差”作为最终入池金额默认填入文本框。例如系统抽水为 `3437` 元、正误差为 `5` 元时，文本框默认显示 `3442` 元。录入用户可以手动修改并保存最终入池金额（最多保留2位小数，扣除误差后的抽水部分仍须为整数），单场和分组水池以输入金额为准；系统会在后端扣除误差后保存对应抽水，避免把误差重复加减。玩家逐项明细仍保留按抽水比例计算的结果供核对。如果之后修改玩家、买入、结算或抽水比例，旧的手动金额会自动清除并重新计算。

全部玩家结算完成后，系统会自动把账目误差并入水池：

```text
结算误差 = 总买入 - 抽水前结算总额
系统入池金额 = 盈利玩家抽水合计 + 结算误差
最终水池 = 手动保存的最终入池金额（未手动保存时为系统入池金额）
```
